...
public class JwtAuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        boolean isPublic = path.contains("/api/accounts/login") 
            ...
            || path.contains("/api/accounts/forgot-password")
            || path.contains("/api/accounts/reset-password");

        System.out.println("GATEWAY DEBUG: Path=" + path + " | Method=" + method + " | isPublic=" + isPublic);

        if (isPublic && !request.getHeaders().containsKey("Authorization")) {
