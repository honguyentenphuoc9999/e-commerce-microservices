...
const AdminCategories = () => {
  ...
  const [formData, setFormData] = useState<Partial<Category>>({ categoryName: "", description: "" });
                ...
                <tr key={cat.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-5">
                      <div className="w-12 h-12 rounded-2xl bg-[#0b1326] flex items-center justify-center border border-white/5 group-hover:border-[#e9c349]/30 transition-all font-bold shadow-xl text-[#e9c349]">
                        <Diamond size={18} />
                      </div>
           ...
           <form onSubmit={handleSubmit} className="bg-[#131b2e] w-full max-w-2xl rounded-3xl border border-white/10 shadow-3xl overflow-hidden animate-in zoom-in-95 duration-500 scale-100">
              ...
              <div className="p-12 space-y-10">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                   ...
                   <div className="space-y-3">
                      <label className="text-[11px] font-black uppercase tracking-widest text-[#e9c349]">Hỗ trợ giao diện</label>
                      <div className="flex gap-3">
                        {[<Diamond key="1" />, <Watch key="2" />, <Zap key="3" />, <Palette key="4" />].map((icon, idx) => (
                           <div 
                             key={idx}
                             className={`w-14 h-14 rounded-xl flex items-center justify-center border transition-all ${idx === 0 ? 'bg-[#e9c349] text-[#0b1326] border-[#e9c349]' : 'bg-white/5 text-slate-500 border-white/5 opacity-50'}`}
                           >
                 ...
                 <div className="space-y-3">
                    ...
                    <textarea 
                      ...
                      placeholder="Mô tả tầm nhìn của danh mục này trong hệ sinh thái Digital Atelier..."
                      rows={5}
                      className="w-full bg-[#0b1326] border border-white/5 rounded-2xl p-8 text-sm text-white focus:ring-1 focus:ring-[#e9c349]/40 outline-none transition-all placeholder:text-slate-800 font-body leading-relaxed italic"
                    />
                 </div>
              </div>
              ...
              <div className="p-10 border-t border-white/5 bg-[#171f33]/30 flex justify-end gap-8">
                 ...
                 <button 
                   ...
                   type="submit"
                   className={`px-14 py-5 bg-[#e9c349] text-[#0b1326] rounded-2xl font-headline font-black text-xs uppercase tracking-widest shadow-3xl shadow-[#e9c349]/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-4 ${categoryMutation.isPending ? 'opacity-50' : ''}`}
                 >
                    {categoryMutation.isPending ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />}
