...
const AdminCategories = () => {
  ...
  const { data: categoriesPage, isLoading } = useQuery({
    queryKey: ['adminCategories', currentPage],
    queryFn: () => catalogService.getCategories(currentPage, pageSize)
  });
    ...
    <div className="p-12 space-y-12 animate-in fade-in duration-700 relative bg-gradient-to-b from-[#0b1326] to-[#0f172a]">
      ...
      <header className="flex justify-between items-end mb-12">
        <div>
          ...
          <h1 className="text-5xl font-black font-headline tracking-tighter text-white uppercase">Hệ thống danh mục</h1>
                ...
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-4 text-slate-600">
                      <Inbox size={48} />
                      <p className="font-label text-xs uppercase tracking-widest">Hệ thống danh mục chưa được thiết lập</p>
                    </div>
                  </td>
                </tr>
              ) : (
            ...
            <div className="p-10 border-b border-white/5 flex justify-between items-center bg-linear-to-r from-[#171f33] to-[#131b2e]">
              <div>
                <h2 className="text-3xl font-headline font-black text-white uppercase tracking-tight italic">
                  {modalType === "add" ? "Thêm mới danh mục" : "Chỉnh sửa danh mục"}
                </h2>
      ...
      <footer className="pt-24 border-t border-white/5 py-12 flex justify-between items-center opacity-30">
        ...
        <div className="flex gap-10">
          ...
          <span className="text-[9px] font-black uppercase tracking-widest mr-4">PHUOC TECHNO OS</span>
        </div>
      </footer>
    </div>
  );
