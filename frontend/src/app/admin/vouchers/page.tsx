...
const AdminVouchers = () => {
  ...
  const { data: vouchersPage, isLoading } = useQuery({
    queryKey: ['admin-vouchers', currentPage],
    queryFn: () => shopService.adminGetAllVouchers(currentPage, pageSize)
  });

  const vouchers = vouchersPage?.content || [];
  const totalPages = vouchersPage?.totalPages || 0;
  const totalElements = vouchersPage?.totalElements || 0;
  ...
  return (
    <div className="p-12 space-y-12 bg-gradient-to-b from-[#0b1326] to-[#0f172a] animate-in fade-in duration-700 min-h-screen">
            ...
            {isLoading ? (
              <tr>
                <td colSpan={7} className="py-20 text-center">
                  ...
                  <p className="text-xs uppercase font-black text-slate-500 tracking-widest">Đang tải dữ liệu</p>
                </td>
              </tr>
            ) : vouchers.length === 0 ? (
              <tr>
        ...
        {!isLoading && totalPages > 1 && (
          <div className="px-10 py-6 bg-[#171f33]/30 border-t border-white/5 flex items-center justify-between">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 italic">
              Trang {currentPage + 1} / {totalPages} • {totalElements} mã ưu đãi
            </p>
          </div>
        )}
      </section>
      ...
      {/* Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-[#02040a]/95 backdrop-blur-xl animate-in fade-in duration-300">
