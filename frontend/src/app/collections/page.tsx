...
function CollectionsContent() {
  ...
  // Reset page when filter changes - Now handled by URL params naturally
  // but we still want to ensure we don't carry over page numbers between categories unless intended.
  // In our case, getFilterUrl will handle it.
  ...
  const getFilterUrl = (newParams: Record<string, string | number | null>) => {
    ...
    // Nếu đổi category hoặc name thì reset về trang 0
    if ('category' in newParams || 'name' in newParams) {
  ...
  const { data: categoriesData = [] } = useQuery({
    queryKey: ['categories'],
    queryFn: () => catalogService.getCategories(0, 100) // Lấy nhiều category để filter
  });

  const { data: productsPage, isLoading, isPlaceholderData } = useQuery({
    queryKey: ['products', categoryParam, nameParam, currentPage, sortBy],
    queryFn: () => catalogService.getProducts({
      category: categoryParam || undefined,
      name: nameParam || undefined,
      page: currentPage,
      size: pageSize,
      sort: sortBy !== "relevant" ? sortBy : undefined
    }),
    placeholderData: (previousData) => previousData, // Giữ dữ liệu cũ khi đang tải trang mới
  });

  const productsData = productsPage?.content || [];
  const totalPages = productsPage?.totalPages || 0;
  const totalElements = productsPage?.totalElements || 0;

  // Fetch TẤT CẢ sản phẩm khớp với từ khóa (để lấy category list đầy đủ)
  const { data: allSearchPage } = useQuery({
  ...
  const allSearchProducts = allSearchPage?.content || [];

  // Extract unique categories from all search results
  const relevantCategories = React.useMemo(() => {
    const source = (nameParam ? allSearchProducts : (categoriesData.content || categoriesData));
  ...
  // Sort productsData - Now handled by backend
  const sortedProducts = productsData;

  const handlePageChange = (newPage: number) => {
    if (newPage >= 0 && newPage < totalPages) {
      router.replace(getFilterUrl({ page: newPage }));
      ...
      <main className="pt-28 pb-20 max-w-[1440px] mx-auto px-8">
        ...
        <section className="flex-grow">
          ...
          {isLoading && !productsData.length ? (
            ...
            <>
              <div
                key={`${categoryParam}-${nameParam}-${currentPage}`}
                className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-12 transition-all duration-500 ${isPlaceholderData ? 'opacity-40 grayscale-[0.5]' : 'opacity-100 animate-in fade-in slide-in-from-bottom-6'}`}
              >
                ...
                <div className="mt-20 flex flex-col items-center gap-8">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="p-4 bg-[#131b2e] border border-white/5 rounded-2xl text-[#dae2fd] hover:text-[#e9c349] hover:border-[#e9c349]/50 disabled:opacity-20 transition-all shadow-xl"
                    >
                      <ChevronLeft size={20} />
                    </button>
                    ...
                    <div className="flex gap-2 mx-4">
                      {[...Array(totalPages)].map((_, i) => (
                        <button
                          ...
                          className={`w-12 h-12 rounded-2xl font-bold text-sm transition-all border ${currentPage === i
                              ? "bg-[#e9c349] border-[#e9c349] text-[#0b1326] shadow-2xl shadow-[#e9c349]/40 scale-110"
                              : "bg-[#131b2e] border-white/5 text-[#c6c6cd] hover:border-[#e9c349]/50 hover:text-white"
                            }`}
                        >
                          {i + 1}
                        </button>
