...
const LoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const actualUserName = typeof response === "string" ? userName : response.userName || userName;
      
      if (token) {
        setAuth(token, actualUserName, id, role, firstName, lastName);
        
        // GỘP GIỎ HÀNG SAU KHI ĐĂNG NHẬP THÀNH CÔNG
        try {
          await shopService.mergeCart();
          console.log("Giỏ hàng đã được gộp thành công.");
        } catch (mergeErr) {
          console.error("Lỗi khi gộp giỏ hàng:", mergeErr);
        }

        if (role === "ROLE_ADMIN" || actualUserName === "admin_tong") {
          router.push("/admin"); 
        } else {
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex flex-col">
      <Header />
      <div className="flex-grow flex items-center justify-center p-6 relative overflow-hidden pt-32 pb-24">
        ...
        >
          <div className="text-center mb-12 group">
            <h1 className="text-4xl font-bold tracking-tight mb-4 text-white">Chào mừng trở lại</h1>
          ...
          <form className="space-y-8" onSubmit={handleLogin}>
            ...
            <div>
              <label className="block text-xs font-bold tracking-widest text-slate-500 mb-3 uppercase">Tài khoản</label>
              <input 
                type="text" 
                value={userName}
