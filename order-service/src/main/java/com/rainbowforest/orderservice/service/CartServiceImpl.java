...
public class CartServiceImpl implements CartService {
    ...
    public void changeItemQuantity(String cartId, Long productId, Integer quantity) {
        ...
        for(Item item : cart){
            if((item.getProduct().getId()).equals(productId)){
                ...
                // Giới hạn bởi số lượng tồn kho (bỏ giới hạn 20 cũ)
                int maxAllowed = item.getProduct().getAvailability();
    ...
    public void mergeCart(String guestCartId, String userCartId) {
        if (guestCartId == null || userCartId == null || guestCartId.equals(userCartId)) return;
        
        List<Item> guestItems = getAllItemsFromCart(guestCartId);
        if (guestItems == null || guestItems.isEmpty()) return;
        
        for (Item item : guestItems) {
            addItemToCart(userCartId, item.getProduct().getId(), item.getQuantity());
        }
        
        // Sau khi gộp xong thì xóa giỏ hàng khách
